...
@IBDesignable open class ActiveLabel: UILabel {
    ...
    open var customBackgroundColor: [ActiveType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    fileprivate func addLinkAttribute(_ mutAttrString: NSMutableAttributedString) {
        ...
        for (type, elements) in activeElements {
            ...
            case .custom:
                ...
                attributes[NSAttributedStringKey.backgroundColor] = customBackgroundColor[type] ?? defaultCustomColor
            }
            ...
            for element in elements {
                let range = NSMakeRange(element.range.location + 1, element.range.length - 1)
